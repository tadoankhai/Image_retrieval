...
from utils.retrieval import return_img_to_img, return_imgcrop_to_img, return_imgtext_to_img
...
def return_id(img_path=None):
    if img_path is not None:
        ...
        score, idx, text = return_imgtext_to_img(img_path, top_k=config.TOP_K)
    else:
        return [], [], None
    return score, idx, crop_image, text
...
with col2:
    if submitted_image and input_image is not None:
        scores, indices, crop_image, text = return_id(img_path=input_image)
